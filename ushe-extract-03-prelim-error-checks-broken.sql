 ...
 -- One-Time Fixes ------------------------------------------------------------------------------------------
 /*
    UPDATE students_current SET s_hsgrad_dt = '20150527', s_entry_action = 'FF' 
    WHERE  s_pidm = '283111';
    
    UPDATE students_current SET s_county_origin = 'UT099', s_state_origin = 'MA', s_country_origin = 'US' 
    WHERE  s_pidm = '281382';
    
    UPDATE students_current SET s_entry_action = 'HS', s_hsgrad_dt = '20190601'
    WHERE  s_pidm IN ('271545','262836','271101','253218','254218','273490','267997',
                      '227352','265870','253094','253927','254068','273099','273480'); 
                     
    UPDATE students_current SET s_entry_action = 'FH' 
    WHERE  s_pidm = '283249';
    
    UPDATE students_current SET s_county_origin = 'UT097' 
    WHERE  s_pidm IN ('281067','277438');
    
    UPDATE students_current SET s_cur_majr2 = NULL, s_major_desc2 = NULL
    WHERE  s_pidm = '91109349';
    
    UPDATE students_current SET s_cur_majr2 = 'ACCT'
    WHERE  s_pidm IN ('99150','110305','172298','109839','171362');
    
    UPDATE students_current SET s_cur_prgm1 = 'ND-CONC', s_cur_majr1 = 'CONC', s_cur_degc1 = 'ND', s_deg_intent = '0'
    WHERE  s_pidm = '274169';
    
    
    UPDATE courses_current SET c_budget_code = 'BA'
    WHERE  c_crn = '23089';
    
    UPDATE courses_current SET c_program_type = 'A' 
    WHERE  c_crn = '22704';
    
    UPDATE courses_current SET c_enrl = c_enrl-1  WHERE c_crn = '22862';
    UPDATE courses_current SET c_enrl = c_enrl+1  WHERE c_crn = '20002';
    
    UPDATE student_courses_current SET sc_crn = '20002', sc_crs_sec = '40'
    WHERE  sc_pidm = '268926' AND sc_crn = '22862';
 ...
 */ 
 ...
 -- SELECT s_entry_action, stu.*
    ...
    AND    EXISTS 
           (
             SELECT s.dsc_pidm 
             FROM   students03@dscir s, stvterm t
           ...
           -- SELECT * from student_courses@dscir WHERE dsc_pidm = 91110381
 ...
 -- SELECT s_pidm, s_banner_id, s_last_name, s_first_name, s_entry_action, s_high_school, s_hsgrad_dt
    ...
    AND    EXISTS 
           (
             SELECT s.dsc_pidm 
             FROM   students03@dscir s, courses@dscir c, student_courses@dscir sc, stvterm
             WHERE  s_pidm = s.dsc_pidm
             AND    s.banner_term = stvterm_code
             AND    s.dsc_term_code  = sc.dsc_term_code
             AND    c.dsc_term_code  = sc.dsc_term_code
             AND    s.dsc_pidm       = sc.dsc_pidm
 ...
 /* SELECT  c_banner_term, c_crn, c_crs_subject, c_crs_number, c_crs_section, s_banner_id, s_styp, s_hsgrad_dt,
 ...
 -- SELECT tw.s_entry_action as tw_ea, eot.s_entry_action as eot_ea, tw.*, eot.* 
    FROM   students_current eot, students03@dscir tw
    WHERE  s_pidm = dsc_pidm 
 ...
 -- SELECT s_deg_intent, students_current.*   
    FROM   students_current
 ...
 /* SELECT s_pidm, s_banner_id, s_first_name, s_last_name, s_cur_prgm1, s_deg_intent, s_entry_action, s_hsgrad_dt, s_styp, s_age,
           'HS Student in Non-HSCE Program' AS reason /**/
    FROM   students_current
    WHERE  s_deg_intent != '0' 
 ...
 */
 ...
 -- SELECT s_hsgrad_dt, s_age, s_county_origin, students_current.*
    FROM   students_current
    WHERE  LENGTH(s_hsgrad_dt) != '8'
    OR    (s_hsgrad_dt IS NULL AND s_age < 21) -- select * from student_courses_current where sc_banner_id = '00411220'
 ...
 -- S-43b ------------------------------------------------------------------------------------------- 
 -- Matching GPA - See how many students have the same term gpa and cumulative gpa.
    -- gpa groups are calculated separately, matches are purely coincidental.
  
    SELECT 'S-43b' AS label, 0 AS err_count FROM dual
    
    SELECT 'S-46*' AS label, count(*) AS err_count
 -- SELECT distinct s_cur_prgm1
    FROM   students_current
 UNION
...
-- UNION
 ...
 /* SELECT sc_banner_term, sc_crn, sc_crs_sbj, sc_crs_num, sc_crs_sec, c_instruct_type, c_site_type, c_budget_code, 
           sc_pidm, s_entry_action, s_hsgrad_dt, sc_stud_type,
