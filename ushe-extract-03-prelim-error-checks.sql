 ...
 SET    s_entry_action   = 'FH',
        s_cur_cip1       = (SELECT s_cur_cip1       FROM students_201943@proddb WHERE s_pidm = pidm),
        s_cur_cip_ushe   = (SELECT s_cur_cip_ushe   FROM students_201943@proddb WHERE s_pidm = pidm),
        s_cur_coll_code1 = (SELECT s_cur_coll_code1 FROM students_201943@proddb WHERE s_pidm = pidm),
        s_cur_degc1      = (SELECT s_cur_degc1      FROM students_201943@proddb WHERE s_pidm = pidm),
        s_deg_intent     = (SELECT s_deg_intent     FROM students_201943@proddb WHERE s_pidm = pidm),
        s_cur_majr1      = (SELECT s_cur_majr1      FROM students_201943@proddb WHERE s_pidm = pidm),
        s_major_desc1    = (SELECT s_major_desc1    FROM students_201943@proddb WHERE s_pidm = pidm),
        s_cur_prgm1      = (SELECT s_cur_prgm1      FROM students_201943@proddb WHERE s_pidm = pidm)
 WHERE  s_pidm IN ('290590','268436','224081','226851','224906','276747');
 
 UPDATE students_current SET s_entry_action = 'RS' WHERE s_pidm = '125714';
 
 DELETE FROM students_current        WHERE s_pidm  = '294588';
 DELETE FROM courses_current         WHERE c_crn   = '30393';
 DELETE FROM student_courses_current WHERE sc_pidm = '294588' AND sc_crn = '30393';
 ...
 */ 
 ...
 -- SELECT s_pidm, s_banner_id, s_last_name, s_first_name, s_entry_action, s_high_school, s_hsgrad_dt
    ...
    AND    EXISTS 
           (
             SELECT s.dsc_pidm 
             FROM   students03@dscir s, stvterm
             WHERE  s_pidm              = dsc_pidm
             AND    banner_term         = stvterm_code
             AND    s_extract           = 'E'
             AND    s_term             != '1'           
             AND    s_entry_action NOT IN ('HS')
 ...
 /* SELECT s_pidm, s_banner_id, s_first_name, s_last_name, s_cur_prgm1, s_deg_intent, s_entry_action, s_hsgrad_dt, s_styp, s_age,
           'Invalid Degree Intent or Entry Action' AS reason, f_calc_entry_action_2(s_pidm, s_banner_term) ea  /**/
    FROM   students_current
    WHERE  s_deg_intent != '0' 
 ...
 */
 ...
 -- SELECT s_hsgrad_dt, s_age, s_county_origin, students_current.*
    ...
    OR    (s_hsgrad_dt IS NULL AND s_age < 21) 
    
    -- SELECT * FROM stvterm WHERE stvterm_code = '201930'
...
-- UNION
 ...
 ----------------------------------------------------------------------------------------------------
 ---------------------------------------------------------------------------------------------------- 
 -- select * from dxgrad_current where dxgrad_majr_desc is null
  -- COMMIT; select ( 
